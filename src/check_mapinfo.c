...
/*   Updated: 2025/07/10 17:12:10 by fbouteil         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    check_4_directions(t_mapinfo *info)
{
    check_direction(info->north->path, "NO ", info);
    check_direction(info->south->path, "SO ", info);
    check_direction(info->west->path, "WE ", info);
    check_direction(info->east->path, "EA ", info);
}

void check_mapinfo(t_mapinfo *info)
{
    check_4_directions(info);
    if (check_color(info->floor_color) != 2 || check_color(info->ceiling_color) != 2)
        exit_clean("Wrong Floor or Ceiling color", info, NULL);
